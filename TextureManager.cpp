...
void TextureManager::cleanUp()
{
    ...
    arr.reserve(s);
    for (std::map<const char*, GLuint>::iterator it = m_idMap.begin(); it != m_idMap.end(); ++it)
    {
...
void TextureManager::addPngTexture(const char *imgFile, const char *name)
{
    ...
    // Check that the image's height and width are powers of 2
    if ( (s->w & (s->w - 1)) != 0 )
    {
